...
private:
    ...
    Bnode_leaf* find_leaf_node(Bnode* root, VALUETYPE value);
    
    void inner_node_insert(Bnode_inner* current, int insert_val, Bnode* child);
    
    void split_leaf_insert(Bnode_leaf* current, int insert_val);
    
    void get_common_ancestor(int out_val, Bnode* lhs, Bnode* rhs);
    
    void inner_node_delete(Bnode_inner* parent, int index);
